...
class NYUv2Dataset(AbsDataset):
    ...
    def _get_data_labels(self, idx):
        ...
        for i in range(len(self.task_name)):
            if self.task_name[i] == 'segmentation':
                label = np.load(os.path.join(self.path, 'label', '{:d}.npy'.format(index)))
            else:
                label = np.moveaxis(np.load(os.path.join(self.path, self.task_name[i], '{:d}.npy'.format(index))), -1, 0)
            labels[i] = torch.from_numpy(label).float()
        return image, labels
